...
export class UserListsService {
  ...
  getUserFavoriteList(user: string){
    return this.http.get<SingleFilmInfo[]>(this.BackEndURL+"/GetVideogameInPreferredList/", {params : {User : user}});
  }

  getUserWishList(user: string){
    return this.http.get<SingleFilmInfo[]>(this.BackEndURL+"/GetVideogameInWishList/", {params : {User : user}});
  }

  isVideogameInFavoriteList(videogameId : number){
    //verifica che il film si trovi nella lista dei favoriti dell'utente corrente
  ...
  isVideogameInWishList(videogameId : number){
    //verifica che il film si trovi nella lista dei desideri dell'utente corrente
